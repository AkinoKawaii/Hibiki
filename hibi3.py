...
async def on_member_update(before, after):
        ...
        if before.name != after.name:
            ...
            embed.set_author(name='{0.mention} Name changed', icon_url=member.avatar_url)
        ...
        if before.nick != after.nick:
            ...
            embed.set_author(name='{0.mention} Nickname changed', icon_url=member.avatar_url)
            await client.send_message(after.server, embed=embed)
            ...
            if len(before.roles) > len(after.roles):
                for role in before.roles:
                    if role not in after.roles:
                        ...
                        embed.set_author(name='{0.mention} Role removed', icon_url=member.avatar_url)
                        await client.send_message(after.server, embed=embed)
            elif len(before.roles) < len(after.roles):
                for role in after.roles:
                    if role not in before.roles:
                        embed = discord.Embed(description='{0.display_name} ({0.id}) got the {1.name} role'.format(before, role))
                        embed.set_author(name='{0.mention} Role applied', icon_url=member.avatar_url)
                        await client.send_message(after.server, embed=embed)
